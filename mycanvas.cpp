...
void MyCanvas::Init(){
    ...
    QHBoxLayout* traverseLayout1 = new QHBoxLayout(traverseBar1);
    traverseBar1->setLayout(traverseLayout1);
    traverseLayout1->setContentsMargins(0, 0, 0, 0);
        
    startKruscal = new textButton("Start Kruscal", vexInfoPage);
    startPrim = new textButton("Start Prim", vexInfoPage);
    traverseLayout1->addWidget(startKruscal);
    traverseLayout1->addWidget(startPrim);
    ...
    textButton* startPrt = new textButton("Print Path", vexInfoPage);
    startDij->setSizePolicy(QSizePolicy::Expanding, QSizePolicy::Preferred);
    startPrt->setSizePolicy(QSizePolicy::Expanding, QSizePolicy::Preferred);

    startPrt->setVisible(false);

    vexInfoLayout->addWidget(startDij);
    vexInfoLayout->addWidget(startPrt);
    ...
    connect(startKruscal, &textButton::clicked, this, [=]() {
        viewLog* newLog = new viewLog("[Kruscal] | --- Kruscal start ---");
        newLog->setStyleSheet("color:#0078d4");
        logDisplay->addWidget(newLog);
        g->Kruscal();
        view->hasVisitedItem = true;
    ...
    connect(startPrt, &textButton::clicked, this, [=]() {
        viewLog* newLog = new viewLog("[Path] | --- Print Path ---");
        newLog->setStyleSheet("color:#0078d4");
        logDisplay->addWidget(newLog);
        g->printPath(view->selectedVex());
        view->hasVisitedItem = true;
