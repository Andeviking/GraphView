        ...
        for (ALArc* p = vexList[minVexID].firstArc; p != nullptr; p = p->nextArc) {
            if (!vexList[p->eVexID].visited) {
                ...
                if (vexList[p->eVexID].info->distance == VexInfo::INF ||
                    vexList[p->eVexID].info->distance > p->weight) {
...
void ALGraph::Kruscal() {
    ClearVisit();
    ResetDistance();
    vector<E<ALArc*>>v;

    for (int i = 0; i < vexList.size(); i++) {
        for (int j = i + 1; j < vexList.size(); j++) {
            ALArc* edge = FindArc(i, j);
            if (edge == nullptr)
                continue;
    ...
    sort(v.begin(), v.end(), [](const E<ALArc*>& a, const E<ALArc*>& b) {
        return a.ptr->weight < b.ptr->weight;
    });
    vector<int>dsu;
    dsu.resize(vexList.size() + 5);
    for (int i = 0; i <= vexList.size(); ++i)
        dsu[i] = i;
    auto get = [&dsu](const int& x) {
        std::function<int(const int&)>func;
        func = [&dsu, &func](const int& x) {
            if (x == dsu[x])
                return x;
            return dsu[x] = func(dsu[x]);
        };
...
void ALGraph::printPath(int strtID)
{
    ClearVisit();
    for (auto& c : vexList)
        if(c.info->distance)
            c.info->gVex->access("", false);
    vector<int>id;
    int now = strtID;
    vexList[strtID].access("end");
    while (vexList[now].info->distance){
        id.push_back(now);
        now = vexList[now].info->preVexID;
    }
...
//    ResetDistance();
    ...
    outVexList[strtID].access("start");
    while (true) {
        //Find next
        int minVexID = -1;
        for (int i = 0; i < outVexList.size(); i++) {
        ...
        for (AMLArc* p = outVexList[minVexID].firstArc; p != nullptr; p = p->outVexID == strtID ? p->nextOutArc : p->nextInArc) {
            ...
            if (!outVexList[endID].visited) {
                ...
                if (outVexList[endID].info->distance == VexInfo::INF ||
                    outVexList[endID].info->distance >p->weight) {
...
void AMLGraph::Kruscal() {
    ClearVisit();
    ResetDistance();
    vector<E<AMLArc*>>v;

    for (int i = 0; i < outVexList.size(); i++) {
        for (int j = i + 1; j < outVexList.size(); j++) {
            AMLArc* edge = FindArc(i, j);
            if (edge == nullptr)
                continue;
    ...
    sort(v.begin(), v.end(), [](const E<AMLArc*>& a, const E<AMLArc*>& b) {
        return a.ptr->weight < b.ptr->weight;
        });
    vector<int>dsu;
    dsu.resize(outVexList.size() + 5);
    for (int i = 0; i <= outVexList.size(); ++i)
        dsu[i] = i;
    auto get = [&dsu](const int& x) {
        std::function<int(const int&)>func;
    ...
    for (const auto& c : v) {
        ...
        FindArc(c.start, c.end)->access();
        if (get(a) == get(b))
            continue;

        dsu[b] = a;
        outVexList[c.start].visit();
        c.ptr->visit();
        outVexList[c.end].visit();
    }
    return;
}

void AMLGraph::printPath(int strtID)
{
    ClearVisit();
    for (auto& c : outVexList)
        if (c.info->distance)
            c.info->gVex->access("", false);
    vector<int>id;
    int now = strtID;
    outVexList[strtID].access("end");
    while (outVexList[now].info->distance) {
        id.push_back(now);
        now = outVexList[now].info->preVexID;
    }

    id.push_back(now);
    reverse(id.begin(), id.end());
    outVexList[id[0]].visit();
    for (int i = 1; i < id.size(); ++i) {
        FindArc(id[i - 1], id[i])->visit();
        outVexList[id[i]].visit();
    }
    return;
}
